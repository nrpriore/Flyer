...
#region // Set/Get
	...
	void Update () {
		...
		if(_settingsTimer >= 1f) {
			_settingsExpand = false;
			_settingsCollapse = false;
		}else if(_settingsExpand) {
			_settingsTimer += Time.deltaTime;
			_settingsRT.localScale = new Vector3(
				Mathf.Lerp(_settingsRT.localScale.x,1f,_settingsTimer),
				Mathf.Lerp(_settingsRT.localScale.y,1f,_settingsTimer),
				1f);
		}else if(_settingsCollapse) {
			_settingsTimer += Time.deltaTime;
			_settingsRT.localScale = new Vector3(
				Mathf.Lerp(_settingsRT.localScale.x,0f,_settingsTimer),
				Mathf.Lerp(_settingsRT.localScale.y,0f,_settingsTimer),
				1f);
		}
	}
	...
	private void SpawnCloud() {
		float _y = -Camera.main.orthographicSize * 1.2f;
		float _x = GetNextX();
